...
function EditorView({ roomId, userName, userColor, password, onLeave }: {
  ...
  const { connected, users, comments, changes, trackChanges, setTrackChanges, bindEditor, addComment, resolveComment, acceptChange, rejectChange } = useCollab(roomId, userName, userColor, password)
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', padding: 10, gap: 10 }}>
        {viewMode !== 'preview' && (
          <div className="editor-panel" style={{ ...panelStyle, flexDirection: 'column' }}>
            <Editor onMount={handleMount} onContentChange={setContent} fontSize={fontSize} comments={comments} changes={changes} />
            {/* Font size controls */}
